...
- [X] T019 [US4] Test: extend `ConversationHistoryTests`/`PromptBuilderTests` with a **>N pairs** case asserting only the last `HistoryPairs` pairs reach the prompt; document the `HistoryPairs` default — in `tests/RagBook.Domain.Tests`/`tests/RagBook.Application.Tests`.
...
- [X] T020 [US5] Integration test: a conversation created in session A → `GET /{id}` and `DELETE /{id}` as session B both return **404** (no existence disclosure), on both tables — in `tests/RagBook.Api.IntegrationTests/Chat/ConversationIsolationTests.cs`.
